...
/* harmony default export */ __webpack_exports__["default"] = ({
  data: function data() {
    ...
    var sizePass = function sizePass(rule, value, callback) {
      if (value === '') {
  ...
  methods: {
    submitForm: function submitForm(formName) {
      ...
      this.$refs[formName].validate(function (valid) {
        if (valid) {
          _this2.axios.post('/api/register', {
            email: _this2.ruleForm.email,
            password: _this2.ruleForm.pass
          }).then(function (response) {
            _this2.MessageSuccess('Пользователь ' + response.data.email + ' успешно зарегистрирован');
            ...
            for (var err in errors) {
              errors[err].forEach(function (e, i) {
                setTimeout(function () {
                  _this2.MessageError(e);
                }, 100 * ++i);
              });
            }
          });
        } else {
          _this2.MessageError('Проверьте правильность заполнения полей');

          return false;
        }
      });
    },
    MessageError: function MessageError(message) {
      this.$message.error(message);
    },
    MessageSuccess: function MessageSuccess(message) {
      this.$message({
        message: message,
        type: 'success'
      });
