            ...
            <el-row id="transaction.id" type="flex" justify="center">
                <el-col :span="18">
                    <el-row :gutter="10" class="tran-row-data">
                        <el-col :span="8">
                            <div>{{ from(transaction) }}</div>
                        </el-col>
                        <el-col :span="8">
                            <div>{{ transaction.money }} &#8381</div>
                        </el-col>
                        <el-col :span="8">
                            <div>{{ to(transaction) }}</div>
                        </el-col>
                    </el-row>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            from(transaction) {
                if (transaction['type_id'] === this.constants.FROM_INCOME) {
                    return this.incomes.find(item => item.id === transaction.income_id).name;
                } else {
                    return this.wallets.find(item => item.id === transaction.wallet_id_from).name;
                }
                return 'источник не установлен'
            },
            to(transaction) {
                if (transaction['type_id'] === this.constants.FROM_WALLET) {
                    return this.expenses.find(item => item.id === transaction.tag_id).name;
                } else {
                    return this.wallets.find(item => item.id === transaction.wallet_id_to).name;
                }
